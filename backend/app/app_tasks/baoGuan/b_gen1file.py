...
class ExcelASNBuilder:
    ...
    def change_sheet_size(self, max_row: int):
    ...
    def calculate_total_data(self, start_row, data):
        ...
        for index, item in enumerate(show_data):
            if show_data[item] is not False:
    ...
    # 为保持兼容，也提供原 detect 名称（但返回 bytes）
    def detect(self, oridata) -> bytes:
        return self.build(oridata)
...
if __name__ == "__main__":
    ...
    xlsx_bytes = builder.detect(data_dicts)

    # 落地保存（可选）
    with open('1-报关资料样板.xlsx', 'wb') as out:
