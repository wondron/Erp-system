...
class ZhuangXiangBuilder:
    def __init__(self, title="ZhangXiang"):
        ...
        self.row_height = [33, 33, 16.1, 16.1, 16.1, 16.1, 16.1, 16.1, 16.1, 33, 16.1]
        self.col_weight = [16, 39, 16, 16, 22, 22]
    ...
    def get_fix_content(self, heyuestr=" "):
        ...
        titlefont = Font(name="等线", size=12)
        title_alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)
        rowindex = z_tools.record_sheet(self.ws, contente3, rowindex + 2, 1, titlefont, title_alignment)
        titlefont = Font(name="等线", size=9)
        title_alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)
        rowindex = z_tools.record_sheet(self.ws, contente4, rowindex, 1, titlefont, title_alignment)
        ...
        rowindex = z_tools.record_sheet(self.ws, contente5, rowindex, 1, titlefont, title_alignment, border=self.thin_border)
        return rowindex
    ...
    def detect(self, oridata, hetongstr, save_path, gongzhangPath):
        rowNum = self.get_fix_content(hetongstr)
        zhuangxiang_data = self.get_zhuangxiang_data(oridata)
        rowNum = self.input_data(zhuangxiang_data, rowNum)
        rowNum = self.calculate_total_data(rowNum, zhuangxiang_data)
        self.change_sheet_size(rowNum)
        self.insert_image(rowNum, gongzhangPath)
